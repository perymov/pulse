...
if (document.querySelector('.mainslider')) {
	let PageSlider = new Swiper('.mainslider__body', {
		/*
		effect: 'fade',
		autoplay: {
			delay: 3000,
		...
		*/
		...
		// Arrows
		navigation: {
			nextEl: '.mainslider__arrow-next',
			prevEl: '.mainslider__arrow-prev',
		},
		/*
		breakpoints: {
			320: {
...
function animOnScroll(params) {
	animItems.forEach((item) => {
		const animItemsHeight = item.offsetHeight,
			animItemsOffset = offset(item).top,
			animStart = 3;
		...
		if ((pageYOffset > animItemsOffset - animItemPoint) && pageYOffset < (animItemsOffset + animItemsHeight)) {
			...
			if (!item.classList.contains('_anim-no-hide')) { // - отключение повторной анимации!!!
				item.classList.remove('_active');
			}
		}
	});
...
const cardLinkFront = document.querySelectorAll('.card__link_front'),
	cardLinkBack = document.querySelectorAll('.card__link_back'),
	tabsPageBtn = document.querySelectorAll('.tabs-page__btn'),
	cardSubtitle = document.querySelectorAll('.card__subtitle'),
	orderPopupSubtitle = document.querySelector('.order-popup-subtitle'),
...
function toggleCard(item) {
	item.forEach((el, i) => {
		el.addEventListener('click', ((e) => {
			e.preventDefault();
			card[i].classList.toggle('_active');
		}));
	});
}
toggleCard(cardLinkFront);
toggleCard(cardLinkBack);

tabsPageBtn.forEach((item, i) => {
	item.addEventListener('click', (() => {
		orderPopupSubtitle.innerHTML = cardSubtitle[i].textContent;
	}));
});
...
window.addEventListener('scroll', (() => {
	if (window.scrollY > 1600) {
		pageUpBtn.classList.add('_active');
	} else {
		pageUpBtn.classList.remove('_active');
	}
}));
	...
	if (error == 0) {
		...
		if (ajax) {
			...
			if (response.ok) {
				...
				if (result) {
					popup_open(result.result + '-message');
				}
				form_clean(form);
			} else {
				alert("Ошибка");
